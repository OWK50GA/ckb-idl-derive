...
```toml
[dev-dependencies]
ckb-idl-export = { path = "../ckb-idl-derive/ckb-idl-export" }
```
...
complete document. Commit the exact exported bytes—not the macro's flat file—
when deploying an advanced schema.

For safety, deriving a recursive witness removes any previously generated
`CARGO_MANIFEST_DIR/idl.json`. Do not hash a leftover macro-side file for a
nested or union witness. Run the exporter explicitly and bind the exact output
path supplied to `cargo run --example export_idl -- <output>`.

---

## IDL commitment at deployment

For a flat witness, the macro-generated `idl.json` is the interface artifact.
For a recursive witness, the exporter output is the interface artifact. Always
pass that exact chosen file to the commitment tool; never discover an IDL by
